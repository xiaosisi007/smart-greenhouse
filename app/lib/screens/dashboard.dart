...
class _DashboardScreenState extends State<DashboardScreen> {
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            _stateCard('卷帘棉被', _t?.curtainState ?? 'idle',
                _t?.limits.curtainTop ?? false, _t?.limits.curtainBottom ?? false, '到顶', '到底'),
            const SizedBox(height: 8),
            _stateCard('顶部通风', _t?.ventState ?? 'idle',
                _t?.limits.ventOpen ?? false, _t?.limits.ventClosed ?? false, '全开', '全关'),
          ],
        ),
      ),
    );
  ...
  Widget _stateCard(String name, String state, bool limA, bool limB, String labelA, String labelB) {
    ...
    return Card(
      child: ListTile(
        ...
        title: Text(name),
        subtitle: Text(_stateText(state)),
